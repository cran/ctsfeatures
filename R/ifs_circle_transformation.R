...
#'
#' \code{ifs_circle_transformation} constructs the IFS circle transformation of
...
ifs_circle_transformation <- function(series, categories, alpha, beta,
  ...
  ifc_circle_plot <- ggplot2::ggplot(df_plot, ggplot2::aes(x = x, y = y, color = factor(z))) +
    ggplot2::geom_point() + ggplot2::ggtitle(title) + ggplot2::xlab('') + ggplot2::ylab('') +
