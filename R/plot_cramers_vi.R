...
#'
#' \code{plot_cramers} constructs a serial dependence plot of a categorical
...
plot_cramers_vi <- function(series, categories, max_lag = 10, alpha = 0.05, plot = TRUE,
                            title = 'Serial dependence plot', bar_width = 0.12,...) {

  x <- y <- NULL
  check_cts(series)
  auxiliary_list <- auxiliary_cramers_vi(series = series, categories = categories,
  ...
  plot_cramers_vi <- ggplot2::ggplot(data = df_plot_1, mapping = ggplot2::aes(x = x, y = y)) +
